...
Write-Host "Preparando backend para Render + Aiven + Gemini..." -ForegroundColor Cyan

Write-TextFile "src/main/resources/application.yml" @'
server:
  port: ${PORT:8080}
...
spring:
  ...
  datasource:
    url: ${DATABASE_URL:jdbc:postgresql://localhost:5432/pastry3d}
...
app:
  jwt:
    secret: ${JWT_SECRET:CHANGE_ME_IN_RENDER_USE_A_LONG_RANDOM_SECRET}
...
DATABASE_URL=jdbc:postgresql://HOST:PORT/defaultdb?sslmode=require
DATABASE_USERNAME=avnadmin
DATABASE_PASSWORD=YOUR_AIVEN_PASSWORD

JWT_SECRET=CHANGE_THIS_TO_A_LONG_RANDOM_SECRET_MINIMUM_64_CHARS
...
Write-TextFile "src/main/java/com/sebastian/pastry3ddemo/service/ai/GeminiRecipeService.java" @'
...
Write-Host ""
Write-Host "Backend preparado para despliegue." -ForegroundColor Green
